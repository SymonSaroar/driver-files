...
static DWORD PLX_Init(WDC_DEVICE_HANDLE *phDev)
{
    ...
    {
        *phDev = PLX_DeviceOpen(P9080_DEFAULT_VENDOR_ID,
...
static DWORD MenuMainExitCb(PVOID pCbCtx)
{
    ...
    if (*(pPlxMenuCtx->phDev) && !PLX_DeviceClose(*(pPlxMenuCtx->phDev)))
        PLX_DIAG_ERR("p9080_diag: Failed closing PLX device: %s",
            PLX_GetLastErr());
    ...
    PLX_DIAG_DMA *pDma)
{
    ...
    strcpy(menuRoot.cTitleName, "PLX 9080 main menu");
    ...
    PLX_DIAG_MenuDmaInit(&menuRoot, phDev, pDma, DiagDmaIntHandler, FALSE);
    PLX_DIAG_MenuEventsInit(&menuRoot, phDev, DiagEventHandler);
    PLX_DIAG_MenuEEPROMInit(&menuRoot, phDev, BIT5);
    ...
    PLX_INT_RESULT *pIntResult)
{
    ...
    printf("Got DMA interrupt, counter %d, INTCSR 0x%X, DMACSR 0x%X\n",
