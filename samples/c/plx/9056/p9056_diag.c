...
#define P9056_DEFAULT_VENDOR_ID 0x10b5 /* Vendor ID */
    ...
    #define P9056_DEFAULT_DEVICE_ID 0x5601 /* Device ID */
#else
...
static DWORD PLX_Init(WDC_DEVICE_HANDLE *phDev)
{
    ...
    {
        *phDev = PLX_DeviceOpen(P9056_DEFAULT_VENDOR_ID,
...
static DWORD MenuMainExitCb(PVOID pCbCtx)
{
    ...
    if (*(pPlxMenuCtx->phDev) && !PLX_DeviceClose(*(pPlxMenuCtx->phDev)))
        PLX_DIAG_ERR("p9056_diag: Failed closing PLX device: %s",
            PLX_GetLastErr());
    ...
    PLX_DIAG_DMA *pDma)
{
    ...
    strcpy(menuRoot.cTitleName, "PLX 9056 main menu");
