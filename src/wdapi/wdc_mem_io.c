...
DWORD DLLCALLCONV WDC_ReadAddr##bits(_In_ WDC_DEVICE_HANDLE hDev, \
    _In_ DWORD dwAddrSpace, _In_ KPTR dwOffset, _Out_ U##bits *val) \
{ \
...
DWORD DLLCALLCONV WDC_WriteAddr##bits(_In_ WDC_DEVICE_HANDLE hDev, \
    _In_ DWORD dwAddrSpace, _In_ KPTR dwOffset, _In_ U##bits val) \
{ \
    DWORD dwStatus = WD_STATUS_SUCCESS; \
    WDC_ADDR_DESC *pAddrDesc; \
    \
...
DWORD DLLCALLCONV WDC_WriteAddrBlock(_In_ WDC_DEVICE_HANDLE hDev,
    _In_ DWORD dwAddrSpace, _In_ KPTR dwOffset, _In_ DWORD dwBytes,
    _In_ PVOID pData, _In_ WDC_ADDR_MODE mode, _In_ WDC_ADDR_RW_OPTIONS options)
{
    DWORD dwStatus = WD_STATUS_SUCCESS;
    WDC_ADDR_DESC *pAddrDesc;
...
{
#if defined(DEBUG)
    if (!pTransCmds || !dwNumTrans)
    {
        WDC_Err("WDC_MultiTransfer: Error - %s\n",
            !pTransCmds ? "NULL WD_TRANSFER pointer" : "no transfer commands "
            "(dwNumTrans == 0)");
        return WD_INVALID_PARAMETER;
    }
#endif

    return WD_MultiTransfer(WDC_GetWDHandle(), pTransCmds, dwNumTrans);
}
